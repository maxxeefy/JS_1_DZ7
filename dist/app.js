...
class Game {
     ...
     */
    run() {
        this.score.scoreWin(this.settings.winLength);
    ...
    doTick() {
        ...
        this.score.scoreGame(this.snake.body.length);
        if (this.ifSnakeEatHerself()) {
            return;
        }
        if (this.isGameWon()) {
            return;
    ...
    ifSnakeEatHerself() {
        let snakeBody = this.snake.body.map(function (cellCoords) {
            return cellCoords.x.toString() + cellCoords.y.toString();
        });
        let head = snakeBody.shift();
        if (snakeBody.includes(head)) {
            clearInterval(this.tickIdentifier);
